...
/**
 * Commandline interface to run the Twitter POS tagger with a variety of
 * possible input and output formats. Also does basic evaluation if given
 * labeled input text.
 * 
...
public class RunTagger {
	...
	String modelFilename = GlobalVariables.modelPath2;
	...
	public static void die(String message) {
		// (BTO) I like "assert false" but assertions are disabled by default in
		// java
	...
	public void runTagger() throws IOException, ClassNotFoundException {
		...
		LineNumberReader reader = new LineNumberReader(
				BasicFileIO.openFileToReadUTF8(inputFilename));
		String line;
		...
		while ((line = reader.readLine()) != null) {
			...
			if (inputFormat.equals("json")) {
				...
				if (text == null) {
					System.err
							.println("Warning, null text (JSON parse error?), using blank string instead");
					text = "";
				}
			} else {
				text = tweetData;
			...
			if (sentence.T() > 0 && !justTokenize) {
				...
				tagger.featureExtractor
						.computeFeatures(sentence, modelSentence);
			...
			if (outputFormat.equals("conll")) {
				...
				outputPrependedTagging(sentence, modelSentence, justTokenize,
						line);
			}
			numtoks += sentence.T();
		}
		...
		System.err
				.printf("Tokenized%s %d tweets (%d tokens) in %.1f seconds: %.1f tweets/sec, %.1f tokens/sec\n",
						justTokenize ? "" : " and tagged",
						reader.getLineNumber(), numtoks,
						(finishtime - currenttime) / 1000.0,
	...
	public void runTaggerInEvalMode() throws IOException,
		...
		System.err.printf("%d tweets in %.1f seconds, %.1f tweets/sec\n", n,
				elapsed, n * 1.0 / elapsed);

		/*
		 * System.err.printf("%d / %d cluster words correct = %.4f acc, %.4f err\n"
		 * , oovTokensCorrect, oovTokens, oovTokensCorrect*1.0 / oovTokens, 1 -
		 ...
		 * .labelVocab.name(i)+"\t"+Arrays.toString(row)); i++; }
		 */
	}
	...
	private void getconfusion(Sentence lSent, ModelSentence mSent,
			int[][] confusion) {
		for (int t = 0; t < mSent.T; t++) {
			int trueLabel = tagger.model.labelVocab.num(lSent.labels.get(t));
			int predLabel = mSent.labels[t];
	 ...
	 * @param mSent
	 * @param inputLine
	 *            -- assume does NOT have trailing newline. (default from java's
	 *            readLine)
	 */
	...
	public static void main(String[] args) throws IOException,
		...
		while (i < args.length) {
			if (!args[i].startsWith("-")) {
				...
				if (args[i + 1].equals("viterbi"))
					tagger.decoder = Decoder.VITERBI;
				else if (args[i + 1].equals("greedy"))
					tagger.decoder = Decoder.GREEDY;
				else
					die("unknown decoder " + args[i + 1]);
				i += 2;
		...
		if (args.length == i || args[i].equals("-")) {
			...
			tagger.inputFilename = GlobalVariables.inputTweet;
		} else {
	...
	public void finalizeOptions() throws IOException {
		...
		if (showConfidence && decoder == Decoder.VITERBI) {
			System.err
					.println("Confidence output is unimplemented in Viterbi, turning it off.");
	...
	public static void usage(String extra) {
		System.out
				.println("RunTagger [options] [ExamplesFilename]"
	...
	public static HashSet<String> wordsInCluster() {
		if (_wordsInCluster == null) {
			_wordsInCluster = new HashSet<String>(
					WordClusterPaths.wordToPath.keySet());
		}
